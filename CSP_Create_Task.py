...
class Dealer:
    ...
    def __str__(self):
        if len(self.dealer_deck) > 0:
            return "The Dealer has {} cards. The cards are {}. Their combined \
            value is {}.\n".format(len(self.dealer_deck), ",\
            ".join([str(i) for i in self.dealer_deck]), self.d_val())
        else:
...
class Player:
    ...
    def __str__(self):
        if not self.player_deck:
            ...
            return """{} has {} cards. The cards are {}. Their combined value is {}.\n""".format(self.name, len(self.player_deck),
                                                                                                 ", ".join([i for i in self.player_deck]), self.p_val())


def hit_stand():
    # user decides hit or stand
    global p_ace
    while True:
        # asks whether they want to hit or stand
    ...
    if hors == "1":
        ...
        print(new_player)
        if 1 in [c.value for c in new_player.player_deck]:
            # check if user has ace after standing
            while True:
                # Take in input until valid value is inputted
                ace = str(input("\nTo make your ace worth 11, press 1, and to\
                make it worth 1, press 0: "))
                if ace not in ["0", "1"]:
                    # valid input check
                    print("Invalid Value. Try again.\n")
                    continue
...
def D_play(d_val):
    global d_ace
    if 1 in [c.value for c in new_dealer.dealer_deck]:
        # ACE IN DECK
        if (d_val + 10 > 16) and (d_val + 10 < 22):
            # dealer can stand by changing ace value
            d_ace = 10
            print("The Dealer has changed their Ace to a value of 11\n")
            print(new_dealer)
            return False
            # dealer changed ace value
        elif (d_val > 16) and (d_val < 22):
            # dealer's hand is already between 16 and 21
            print('The Dealer decides to stand\n')
            return False
            # dealer stands
        elif (d_val > 21):
            # dealer's hand went over 21
            return False
            # dealer went bust
        else:
            # dealer doesn't have to change ace value and can hit another card
            d_card = new_deck.hit()
        ...
        # NO ACE IN DECK
        if d_val < 17:
            # dealer hits whenever their hand value is less than 17
            d_card = new_deck.hit()
            new_dealer.dealer_deck.append(d_card)
            print("The Dealer decided to hit and recieved a {}.\
            \n".format(d_card))
            ...
            # dealer stands if their hand value is in between 16 and 21
            print('The Dealer decides to stand\n')
            return False
            # dealer stands
        elif (d_val > 21):
...
print("""\nWelcome to the game of blackjack! In this game, both you and the dealer
start with two cards each. You can choose either to hit or to stand. Hitting
adds a card to your hand, while standing ends your turn, and you are unable to
hit again. The goal of the game is to have the values of your cards be the
closest to 21 but not over 21. The cards from 2 - 10 have the values that
match themselves, face cards \are all worth 10, and ace can either be worth
1 or 11. If you have an ace in your hand you can change the value of the ace
...
while True:
    ...
    while hit_stand():
        if 1 in [c.value for c in new_player.player_deck]:
            ...
            if not new_player.p_val() > 21 and not p_ace == 0:
                # avoids unecessary input for ace
                while True:
                    # Take in input until valid value is inputted
                    print(new_player)
                    ace = str(input("To make your ace worth 11, press 1, and to\
                    make it worth 1, press 0: \n"))
                    if ace not in ["0", "1"]:
                        # valid input check
                        print("Invalid Value. Try again.\n")
...
# Dealer Plays if player didn't go bust
    ...
    while True:
        ...
        play_again = str(input("\nWould you like to play again (yes/y or \
        no/n)? "))
